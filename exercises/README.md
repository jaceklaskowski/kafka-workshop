# Exercises for Apache Kafka™ and Kafka Streams Workshops

This repository contains the exercises for [Apache Kafka™ and Kafka Streams Workshops](../README.md).

## Topic 1: Apache Kafka™ for Administrators and Operators

1. [Exercise: Installing and Running Kafka](kafka-exercise-Installing-and-Running-Kafka-Broker.md)
1. [Exercise: Configuring Multi-Broker Kafka Cluster](kafka-exercise-Configuring-Multi-Broker-Kafka-Cluster.md)
1. [Exercise: Exploring Zookeeper](kafka-exercise-Exploring-Zookeeper.md)
1. [Exercise: Using Kafka with Docker](kafka-exercise-Using-Kafka-with-Docker.md)
1. [Exercise: Using kafka-topics with Multi-Broker Cluster](kafka-exercise-Using-kafka-topics-with-Multi-Broker-Cluster.md)

## Topic 2: Apache Kafka for Developers

1. [Exercise: Developing Kafka Producer](kafka-exercise-Developing-Kafka-Producer.md)
1. [Exercise: Sending Messages with Callback](kafka-exercise-Sending-Messages-with-Callback.md)
1. [Exercise: Using Custom Partitioner](kafka-exercise-Using-Custom-Partitioner.md)
1. [Exercise: Using Custom Keys](kafka-exercise-Using-Custom-Keys.md)
1. [Exercise: Message Delivery Acknowledgement](kafka-exercise-Message-Delivery-Acknowledgement.md)
1. [Exercise: Developing Kafka Consumer](kafka-exercise-Developing-Kafka-Consumer.md)
1. [Exercise: Kafka Application as Consumer and Producer](kafka-exercise-Kafka-Application-as-Consumer-and-Producer.md)
1. [Exercise: Word Count Per Record](kafka-exercise-Word-Count-Per-Record.md)
1. [Exercise: Global Word Count](kafka-exercise-Global-Word-Count.md)

## Topic 3: Kafka Streams (Scala API)

1. [Exercise: Processing Record Stream with KStream](kafka-streams-exercise-Processing-Record-Stream-with-KStream.md)
